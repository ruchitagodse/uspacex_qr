...
const CreateEvent = () => {
  ...
  const [eventQRCode, setEventQRCode] = useState(null); // State to hold QR code
  ...
  const handleCreateEvent = async (e) => {
    ...
    setError('');
    setSuccess('');
    setEventQRCode(null); // Reset QR code on new submission
    ...
    try {
      ...
      // Create the QR code link for the event page
      const eventLink = `https://uspacex.vercel.app/events/${uniqueId}`;
      setEventQRCode(eventLink); // Set the event QR code link
      ...
      <section className='c-form box'>
        ...
        {eventQRCode && (
          <div className="qr-code-container">
            <h4>Scan the QR code to join the event:</h4>
            <QRCode value={eventQRCode} size={256} /> {/* QR code */}
          </div>
        )}
      </section>
    </>
