...
const ManageEvents = () => {
    ...
    const handleQrCode = (eventId) => {
        router.push(`/admin/event/qr-codes/${eventId}`);
    };
                            ...
                            <tr key={event.id}>
                                ...
                                <td style={{ padding: '10px', border: '1px solid #ddd' }}>
                                    <div className="twobtn">
                                        ...
                                        <button
                                            className="m-button-7"
                                            onClick={() => handleQrCode(event.id)}
                                            style={{ marginLeft: '10px', backgroundColor: '#f16f06', color: 'white' }}
                                        >
                                            Manage QR
                                        </button>
                                        <button className="m-button-7" onClick={() => handleEditEvent(event.id)} style={{ marginLeft: '10px', backgroundColor: '#e2e2e2', color: 'black' }}><CiEdit />Edit</button>
                                    </div>
                                </td>
                            </tr>
                        ))}
